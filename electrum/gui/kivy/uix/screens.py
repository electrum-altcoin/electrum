...
class SendScreen(CScreen):
    ...
    def read_invoice(self):
        ...
        if not address:
            self.app.show_error(_('Recipient not specified.') + ' ' + _('Please scan a {name} address or a payment request'.format(name=constants.net.NAME)))
            return
        if not self.amount:
        ...
        if self.is_lightning:
            ...
            if self.payment_request:
                ...
                if not bitcoin.is_address(address):
                    self.app.show_error(_('Invalid {name} Address').format(name=constants.net.NAME) + ':\n' + address)
                    return
